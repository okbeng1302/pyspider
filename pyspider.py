...
class Handler(BaseHandler):
    ...
    def parse_city_category(self,response):
        ...
        for cd in category_divs:
            ...
            for dl in category_dls:
                # 一级类别
                category_name = dl('dt > a').text()
                # 二级类别
                filter_category_dds = dl('dd > span > a').items()
                for dd in filter_category_dds:
                    # 二级类别
                    filter_category = dd.text()
                    # 二级类别链接
                    ...
                    if f_c_href.find('html?') != -1:
                        ...
                        if f_c_href.find(';') != -1:
                            ...
                            self.crawl(url,callback=self.parse_course1,save={'category_name':category_name,'filter_category':filter_category})                            
                        # 中学
                        elif f_c_href.find('MiddleSchool') != -1:
                            req_url = f_c_href + '&applystate=0&hide=0'
                            self.crawl(req_url,callback=self.parse_course2,save={'category_name':category_name,'filter_category':filter_category})
                        # 课程
                        elif f_c_href.find(r'Course') != -1:
                           # print f_c_href
                            self.crawl(f_c_href,fetch_type='js',callback=self.parse_course3,save={'category_name':category_name,'filter_category':filter_category,'PageNumber':1})
                        # 四个参数  
                        else:
                            # print f_c_href
                            pattern = (r'Category\/(.*?)-(.*?)-(.*?)-(.*?)\.html?')
                            ...
                            if len(url_params) == 0:
                               ...
                               # print url_params
                                ...
                                # print f_c_href
                                self.crawl(f_c_href,callback=self.parse_course4,save={'category_name':category_name,'filter_category':filter_category})
                                # print f_c_href
                    else:
                        #print f_c_href
                        if 'publish' in f_c_href:
                            ...
                            if 'Course' in f_c_href:
                                # print filter_category,f_c_href
                                pattern = (r'-(.*?)-(.*?)\.html')
                                ...
                                self.crawl(f_c_href,fetch_type='js',callback=self.parse_course5,save={'category_name':category_name,'filter_category':filter_category,'PageNumber':1,'Coursecode':Coursecode})
                            else:
                                # print f_c_href
                                if 'kw' in f_c_href:
                                    f_c_href = f_c_href + '&applystate=0&hide=0'
                                    # print f_c_href
                                    self.crawl(f_c_href,callback=self.parse_course6,save={'category_name':category_name,'filter_category':filter_category})
                                else:
                                    if 'bjuu' in f_c_href:
                                        pass
                                    else:
                                        if f_c_href.find('hide') == -1:
                                            f_c_href = f_c_href + '&applystate=0&hide=0'
                                        self.crawl(f_c_href,callback=self.parse_course1,save={'category_name':category_name,'filter_category':filter_category})  
                                            
                                            
                                

                        # pass
                    # print category_name,filter_category,f_c_href
    # 包含检索词
    def parse_course6(self,response):
        ...
        for div in course_list:
            ...
            self.crawl(course_href,fetch_type='js',callback=self.parse_course6_detail,save
                       ={'category_name':response.save['category_name'],'keyword':keyword,'filter_category':response.save['filter_category'],'Coursecode':course_id,'course_name':course_name,'PageNumber':1})      
        # 翻页
        ...
        if next_page == '':
            ...
            self.crawl(next_page,callback = self.parse_course6,save={'category_name':response.save['category_name'],'filter_category':response.save['filter_category']})
    def parse_course6_detail(self,response):
        ...
        # 二级分类
        filter_category = response.save['filter_category']
        # 检索词
        ...
        for div in class_list:
            ...
            # 班级编号
            course_code = div('div[class="m-classlist-l class-list"] >div.m-class-info > h3 > a ').text()
            if course_code == '':
                course_code = div('div[class="m-classlist-l  class-list"] >div.m-class-info > h3 > a ').text()
            ...
            # 课时
            pattern = (r'.*?,共计(.*?)次课')
            lesson_times = re.findall(pattern,p_text.encode('utf-8'))[0]
            print lesson_times
            # 上课地点
            pattern_space = (r'.*?地点： (.*?) ')
            ...
            # 终止时间
            stop_date = start_and_stop_time[1].replace('/','-',2)
            t_s = time.strptime(stop_date, "%Y-%m-%d")
            y_s,m_s,d_s = t_s[0:3]
            end_date = datetime.datetime(y_s,m_s,d_s)
            # print "结课时间:",stop_date
            #　课程持续时间
            last_days = (end_date-start_date).days
            # print "课程持续时间:",last_days
            # 班级容量
        ...
        if next_page == None:
            ...
            print base_next_url
            self.crawl(base_next_url,fetch_type='js',callback=self.parse_course6_detail,save={'PageNumber':PageNumber+1,'city_name':city_name,'cid':cid,'Coursecode':Coursecode,'category_name':category_name,'filter_category':filter_category,'course_name':course_name,'keyword':keyword})
    ...
    def parse_course5(self,response):
        ...
        if next_page == None:
            ...
            print base_next_url
            self.crawl(base_next_url,fetch_type='js',callback=self.parse_course5,save={'PageNumber':PageNumber+1,'city_name':city_name,'cid':cid,'Coursecode':Coursecode,'category_name':category_name,'filter_category':filter_category,'course_name':course_name})
    ...
    def parse_course1(self,response):
        ...
        for div in courses_div:
            ...
            self.crawl(course_href,callback=self.parse_course1_detail,save={'category_name':response.save['category_name'],'filter_category':response.save['filter_category'],'course_name':course_name,'course_code':course_code,'PageNumber':1})
        next_page = response.doc('.m-page > div > a.nextlink').attr.href
        print "下一页",next_page
        if next_page == None:
            next_page = ''
        ...
        if next_page == '':
            ...
            self.crawl(next_page,callback = self.parse_course1,save={'category_name':response.save['category_name'],'filter_category':response.save['filter_category']})
    ...
    def parse_course2(self,response):
        ...
        for div in course_divs:
            ...
            print course_code
            self.crawl(course_href,fetch_type='js',callback=self.parse_course2_detail,save={'category_name':response.save['category_name'],'filter_category':response.save['filter_category'],'course_name':course_name,'course_code':course_code,'PageNumber':1})
            next_page = response.doc('.m-page > div > a.nextlink').attr.href
            print "下一页",next_page
            if next_page == None:
                next_page = ''
            ...
            if next_page == '':
                ...
                self.crawl(next_page,callback = self.parse_course2,save={'category_name':response.save['category_name'],'filter_category':response.save['filter_category']})
    @config(priority=10)       
    def parse_course2_detail(self,response):
        print "访问",response.url
        # 课程 ID
        class_id = response.save['course_code']
        print '班级ID:',type(class_id)
        # print response.text
        ...
        for div in course_list_div:
            # 课程编号
            course_code = div('div[class="m-classlist mt10"] > div > div.m-class-info > h3 > a').text()
            print course_code
            # 时间
            p_text = div('div[class="m-classlist mt10"] > div > div.m-class-info > p').text()
            print p_text
        ...
        if next_page == None:
            ...
            print next_page_url
            self.crawl(next_page_url,callback=self.parse_course2_detail,save={'course_code':class_id,'category_name':response.save['category_name'],'filter_category':response.save['filter_category'],'Coursecode':Coursecode,'course_name':course_name,'PageNumber':PageNumber+1,'city_name':city_name})
    ...
    def parse_course3(self,response):
        ...
        if next_page == None:
            ...
            print base_next_url
            self.crawl(base_next_url,fetch_type='js',callback=self.parse_course3,save={'PageNumber':PageNumber+1,'keyword':keyword,'city_name':city_name,'cid':cid,'Coursecode':Coursecode,'category_name':category_name,'filter_category':filter_category,'course_name':course_name})
    ...
    def parse_course4(self,response):
        ...
        for div in course_div:
            ...
            course_href = div('div[class="m-courselist-l pr"] > h2 > a').attr.href
            print '课程链接',course_href
            self.crawl(course_href,callback=self.parse_course4_detail,save={'category_name':response.save['category_name'],'filter_category':response.save['filter_category'],'course_name':course_name,'course_code':course_code,'PageNumber':1})
        ...
        if next_page == '':
            ...
            self.crawl(next_page,callback = self.parse_course4,save={'category_name':response.save['category_name'],'filter_category':response.save['filter_category']})
    @config(priority=10) 
    def parse_course4_detail(self,response):
        ...
        if next_page == None:
            ...
            print next_page_url
            self.crawl(next_page_url,callback=self.parse_course4_detail,save={'city_name':city_name,'cid':cid,'category_name':category_name,'filter_category':filter_category,'course_name':course_name,'course_code':course_id,'PageNumber':PageNumber+1})
    ...
    def parse_course1_detail(self,response):
        ...
        if next_page == None:
            ...
            self.crawl(req_url,callback=self.parse_course1_detail,save={'city_name':city_name,'cid':cid,'category_name':category_name,'filter_category':filter_category,'course_name':course_name,'course_code':course_id,'PageNumber':PageNumber+1})
    ...
    def on_result(self,result):
        ...
        db = client.research
        web_xdf_courses = db.web_xdf_courses
        if web_xdf_courses.find_one({'course_code':result['course_code']}):
            print '此课程已存在'
        else:
            print  result['course_id'] , type( result['course_id'])
            course = {}
            course['city'] = result['city']
            course['category_name'] = result['category_name']
            course['filter_category'] = result['filter_category']
            ...
            course['start_date'] = result['start_date']
            course['end_date'] = result['end_date']
            course['lesson_times'] = result['lesson_times']
            course['last_days'] = result['last_days']
            course['update_time'] = result['update_time']
            ...
            course['course_id'] = result['course_id'].encode('utf-8'),
            course['course_name'] = result['course_name']
            web_xdf_courses.insert_one(course)
